...
class Inference():
    ...
    def random_synthesis(self, num = 2, aux = False, out_dir = None, show = False):
        ...
        for w, source_image, stylized_image, name in zip(ws, source_images, adapted_images, names):
            ...
            if out_dir is not None:
                common_utils.save_image(image, os.path.join(out_dir, name + f'.jpg'),
                                        nrow = 1, range = '-1,1')
            if show is True:
                common_utils.show_tensor_image(image, nrow = 2)

    @torch.no_grad()
    def transfer(self, image_path, aux = False, out_dir = None, show = False, manipulation = False):
        ...
        for w, image, source_image, stylized_image, name in zip(ws, images, source_images, adapted_images, names):
            ...
            if manipulation is True:
                edited_images = self.semantic_manipulation(w)
            if out_dir is not None:
                common_utils.save_image(image, os.path.join(out_dir, name + f'.jpg'),
                                        nrow = len(image), range = '-1,1')
                common_utils.save_image(edited_images, os.path.join(out_dir, name + f'_edited.jpg'),
                                        nrow = len(edited_images), range = '-1,1')
            if show is True:
                common_utils.show_tensor_image(image, nrow = len(image))
                common_utils.show_tensor_image(edited_images, nrow = len(edited_images))
    ...
    def generate_test_latent(self, w, sample_num = 36, linspace_num = 5):
        ...
        with torch.no_grad():
            ...
            w_samples = self.G.mapping(torch.from_numpy(z_samples).to(self.device), **config.mapping_kwargs)
            ws_lerp = []
    ...
    def semantic_manipulation(self, w, name = ''):
        ...
        for direction, factor_and_edit in latents_after_edit.items():
            ...
            for factor, latent in factor_and_edit.items():
                edited_image= self.G_target(latent, is_latent = True, aux = self.use_aux, alpha = 0.0, **synthesis_kwargs)
...
if __name__ == '__main__':
    ...
    inference.random_synthesis(num = 1, aux = inference.use_aux, out_dir = config.out_dir, show = config.show)
    inference.transfer(config.test_image, aux = inference.use_aux, out_dir = config.out_dir, show = config.show, manipulation = True)