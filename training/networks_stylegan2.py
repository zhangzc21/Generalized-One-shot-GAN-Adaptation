...
def modulated_conv2d(
    ...
    x = conv2d_resample.conv2d_resample(x = x, w = w.to(x.dtype), f = resample_filter, up = up, down = down,
                                        padding = padding, groups = batch_size, flip_weight = flip_weight)
...
class Conv2dLayer(torch.nn.Module):
    ...
    def forward(self, x, gain = 1):
        ...
        x = conv2d_resample.conv2d_resample(x = x, w = w.to(x.dtype), f = self.resample_filter, up = self.up,
...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        if c_dim > 0:
            self.embed = FullyConnectedLayer(c_dim,
                                             embed_features)  # FullyConnectedLayer: fusing bias and activation together
...
class ToRGBLayer(torch.nn.Module):
    ...
    def extra_repr(self):
        return f'in_channels={self.in_channels:update_emas}, out_channels={self.out_channels:d}, w_dim={self.w_dim:d}'
